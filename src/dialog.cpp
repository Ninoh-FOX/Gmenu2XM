...
void Dialog::drawTitleIcon(Surface& s, const std::string &icon, bool skinRes)
{
	...
	i->blit(s, 4, (gmenu2x->skinConfInt["topBarHeight"] - 64) / 2);
}

void Dialog::writeTitle(Surface& s, const std::string &title)
{
	gmenu2x->font->write(s, title, 80, 0, Font::HAlignLeft, Font::VAlignTop);
}

void Dialog::writeSubTitle(Surface& s, const std::string &subtitle)
{
	std::string wrapped = gmenu2x->font->wordWrap(subtitle, gmenu2x->resX - 96);
	gmenu2x->font->write(s, wrapped, 80,
