	...
	, selected(0)
{
	...
	w += 46;
	const int h = (font.getLineSpacing() + 4) * options.size() + 16;
	box = {
...
void ContextMenu::paint(Surface &s)
{
	...
	s.rectangle(box.x + 4, box.y + 4, box.w - 8, box.h - 8,
			gmenu2x.skinConfColors[COLOR_MESSAGE_BOX_BORDER]);

	// Draw selection background.
	const int h = font.getLineSpacing();
	SDL_Rect selbox = {
		static_cast<Sint16>(box.x + 8),
		static_cast<Sint16>(box.y + 8 + (h + 4) * selected),
		static_cast<Uint16>(box.w - 16),
	...
	for (uint i = 0; i < options.size(); i++) {
		font.write(s, options[i]->text, box.x + 24, box.y + 10 + (h + 4) * i,
...
bool ContextMenu::handleTouchscreen(Touchscreen &ts) {
	if (ts.inRect(box)) {
		int i = std::max(0, std::min(static_cast<int>(options.size()) - 1,
				(ts.getY() - (box.y + 8)) / (gmenu2x.font->getLineSpacing() + 4)));
		if (ts.released()) {
			options[i]->action();
			dismiss();
		} else if (ts.pressed()) {
